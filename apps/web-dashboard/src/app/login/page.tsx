...
export default function LoginPage() {
  ...
  function handleLogin(e: React.FormEvent) {
    ...
    if (trimmed.length < 16) {
      setError("API key is too short (min 16 characters)");
      return;
    }
    if (!trimmed.match(/^(ak|sk)_/)) {
      setError("Invalid format. Must start with \"ak_\" or \"sk_\"");
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface-50">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-surface-500 mt-2">Sign in with your API key</p>
        </div>
        ...
        <p className="text-xs text-surface-500 text-center mt-2">
          New here? <a href="/signup" className="text-brand-600 hover:text-brand-700 font-medium">Create workspace</a>
        </p>
      </div>
    </div>
  );
